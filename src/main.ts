...
import { getUser, comparePassword, getShortUser } from "./db"
...
app.get("/get/user", async function (_req: Request, res: Response) {
  res.setHeader('Content-Type', 'application/json');
  const id: number = parseInt(_req.query.id as string);
  let user = "";
...
app.get("/get/login", async function (req: express.Request, res: { setHeader: any; send: (arg0: string) => void; }) {
  ...
  const passwordToTest: string = req.query.password as string;
